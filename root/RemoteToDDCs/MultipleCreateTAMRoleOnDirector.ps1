...
Get-Content C:\batch\CDCnames.txt | ForEach-Object{
$CDCNAME = $_

$CDCNAME
#Enter Poweshell Session on Remote server, add Citrix SnapIn

# new-pssession needs FQDN
$s = New-PSSession -ComputerName $CDCNAME
Invoke-Command -Session $s {$h = Add-PSSnapin citrix*} -ErrorAction SilentlyContinue


#Create Administrator Role
#Invoke-Command -Session $s {$h = New-AdminRole -Name "TAM-DirectorAccess" -Description "RITM001213435 View Only Access for TAMs"} -ErrorAction SilentlyContinue
